...
nav_files = np.sort(glob(os.path.join(nav_path,'*mru_seapath330_navbho*.txt')))
...
lat_corr = sgg.wgs_grav(dgs_data['lat_new']) + sgg.fa_2ord(dgs_data['lat_new'],ellipsoid_ht)
eotvos_corr = sgg.eotvos_full(dgs_data['lon_new'].values, dgs_data['lat_new'].values,
                              ellipsoid_ht, sampling)
tide_corr = sgg.longman_tide_pred(dgs_data['lon_new'],dgs_data['lat_new'],dgs_data['date_time'])
...
for path in dgs_files_raw:  # clean up weird Ride serial files
    ...
    with open(opath,'w') as f:
        for line in long:
            f.write(line)
dgs_files_raw = np.sort(glob(os.path.join(dgs_path_raw,'clean_SR*_grav_dgs*.txt')))

raw_dgs = sgi.read_raw_dgs(dgs_files_raw, ship)

# read some meter info for the raw data calibration
ini_file = os.path.join(root,ship,cruise,'Meter.ini')  # find the meter config file
sgu.clean_ini_to_toml(ini_file)  # make it a little easier to read
meter_conf = tm.load(open(os.path.join(root,ship,cruise,'Meter.toml'),'rb'))
...
d0 = dgs_data.iloc[0]['date_time']; d1 = dgs_data.iloc[500]['date_time']
plt.figure(figsize=(11,4.8))
sl0 = dgs_data.date_time > d0; sl1 = dgs_data.date_time < d1 
plt.plot(dgs_data[(sl0)&(sl1)]['date_time'],dgs_data[(sl0)&(sl1)]['rgrav'],color='k',label='laptop')
sl0 = raw_dgs.date_time > d0; sl1 = raw_dgs.date_time < d1 
plt.plot(raw_dgs[(sl0)&(sl1)]['date_time'],raw_dgs[(sl0)&(sl1)]['grav_cal'],color='r',lw=.5,label='serial')
...
raw_dgs['tsec'] = [e.timestamp() for e in raw_dgs['date_time']]  # posix stamps in raw file
                                                                 # have some dropouts
...
sat_grav = np.loadtxt(os.path.join(root,ship,cruise,'sandwell_tracked.llg'),usecols=(2,))

# plot laptop FAA and satellite data (trim edge effects from filtering)
plt.figure(figsize=(11,4.8))
plt.plot(dgs_data.iloc[taps:-taps//2]['date_time'], ffaa[taps:-taps//2],label='SR2312')
plt.plot(dgs_data.iloc[taps:-taps//2]['date_time'], sat_grav[taps:-taps//2],label='satellite')
plt.xlabel('Timestamp')
