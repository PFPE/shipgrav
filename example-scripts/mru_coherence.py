...
mru_files = np.sort(glob(os.path.join(mru_path,'SR2312_mru_hydrins_navbho*.txt')))
yaml_file = os.path.join(root,ship,cruise,'openrvdas/doc/devices/IXBlue.yaml')
talk = 'PASHR'
dats = []
for mf in mru_files:
...
for motion in ['Pitch','Roll','Heave']:
    ...
    mru_interp = np.interp(dgs_data.tsec, mru_dat.tsec, mru_dat['%s:g' % motion].rolling(1000).max())
