...
if l_seg < l_ext:  # but if it's a short segment, just use the segment length on each end
    l_ext = l_seg
xrng = np.zeros(len(data))  # calculate distances btwn points to get avg spacing for extensions
...
frnt_ext = np.zeros((n_ext,2)); back_ext = np.zeros((n_ext,2))
for i in range(n_ext):
    frnt_pt = wgs.Direct(data.iloc[0]['lat_new'],data.iloc[0]['lon_new'],faz10,dx_avg*(i+1))
    back_pt = wgs.Direct(data.iloc[-1]['lat_new'],data.iloc[-1]['lon_new'],faz01,dx_avg*(i+1))
    frnt_ext[i,0] = frnt_pt['lon2']; frnt_ext[i,1] = frnt_pt['lat2']
    back_ext[i,0] = back_pt['lon2']; back_ext[i,1] = back_pt['lat2']

# compile full coordinate lists for the extended line, write out for grdtrack
a_lon = np.hstack([frnt_ext[:,0][::-1],data['lon_new'].values,back_ext[:,0]])
a_lat = np.hstack([frnt_ext[:,1][::-1],data['lat_new'].values,back_ext[:,1]])
########################################################################
...
# info here
track = read_csv(os.path.join(seg_path,'tracked.llm'),sep='\t',names=['lon','lat','dep','faa','sed','age'])
...
xobs = np.linspace(0,xpts_line[-1],int(1e3))  # NOTE be carefule with the number of points here
...
rho = np.array((1000,2100,2900)) # sed density approx based on Hamilton 1978, doi: 10.1121/1.381747
...
wlarge = 30  # in km; keep these 2 values small for short line (otherwise more like 100/45)
wsmall = 5
