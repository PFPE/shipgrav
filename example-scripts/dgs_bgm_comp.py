...
bgm_path = os.path.join(root,ship,cruise,'gravimeter/BGM3/serial')  # we only have serial BGM
...
for df in [dgs_data, bgm_data]:
    ...
    lat_corr = sgg.wgs_grav(df['lat_new']) + sgg.fa_2ord(df['lat_new'],ellipsoid_ht)
    eotvos_corr = sgg.eotvos_full(df['lon_new'].values, df['lat_new'].values,
                                  ellipsoid_ht, sampling)
    tide_corr = sgg.longman_tide_pred(df['lon_new'],df['lat_new'],df['date_time'])
...
plt.plot(dgs_data.iloc[taps:-taps//2]['date_time'], dfaa[taps:-taps//2],label='TN400 DGS')
plt.plot(bgm_data.iloc[taps:-taps//2]['date_time'], bfaa[taps:-taps//2],label='TN400 BGM')
