    ...
    for fpath in tqdm(pathlist,desc='reading nav',disable=not progressbar):  # loop nav files (may be a lot of them)
        ...
        if ship_function:  # use a user-supplied function to get all the things
            ...
            if ship == 'Atlantis':
                ...
                lon, lat = _navcoords_Sikuliaq(allnav, talker)
                timest = _navdate_Sikuliaq(allnav, talker)
...
def _navdate_NBP(allnav, talker):
    ...
    # In NBP2501, the GPS is using ISO timestamps instead of whatever format above
    # timest = np.array([ pd.to_datetime(line.split(talker)[0].strip()) 
    #                 for line in subnav ])
...
def fix_iso_timestamp(ts):
    # Remove 'Z' suffix
    ts = ts.rstrip('Z')
    # Match fractional seconds part (if any)
    match = re.match(r'(.*\.\d{1,6})(.*)', ts)
    if match:
        main_part = match.group(1)
        rest = match.group(2)
        # Pad fractional seconds to exactly 6 digits
        if '.' in main_part:
            date_part, frac = main_part.split('.')
            frac = (frac + '000000')[:6]  # pad or truncate to 6 digits
            ts_fixed = f"{date_part}.{frac}{rest}+00:00"
            return ts_fixed
    # If no fractional seconds, just append timezone
    return ts + '+00:00'


def _navcoords_Sikuliaq(allnav, talker):
    # Extract lon/lat from Sikuliaq GNSS nav files
    inav = [talker in s for s in allnav]
    subnav = [line for line in allnav if talker in line]
    N = len(subnav)
    if N == 0:
    ...
    for i in range(N):
        try:
            fields = subnav[i].strip().split('\t')
            nmea = fields[-1].split(',')

            lat_val = float(nmea[2])
            lat_deg = int(lat_val / 100)
            lat_min = lat_val - lat_deg * 100
            lat[i] = lat_deg + lat_min / 60
            if nmea[3] == 'S':
                lat[i] = -lat[i]

            lon_val = float(nmea[4])
            lon_deg = int(lon_val / 100)
            lon_min = lon_val - lon_deg * 100
            lon[i] = lon_deg + lon_min / 60
            if nmea[5] == 'W':
                lon[i] = -lon[i]

        except Exception as e:
            print(f"Error parsing nav line: {subnav[i].strip()} | {e}")
            lat[i] = np.nan
            lon[i] = np.nan
...
def _navdate_Sikuliaq(allnav, talker):
    # Extract datetime info from Sikuliaq GNSS nav files
    inav = [talker in s for s in allnav]
    subnav = [line for line in allnav if talker in line]
    N = len(subnav)
    if N == 0:
    ...
    for i in range(N):
        try:
            parts = subnav[i].strip().split('\t')
            raw_ts = parts[1]
            fixed_ts = fix_iso_timestamp(raw_ts)
            timest[i] = datetime.fromisoformat(fixed_ts)
        except Exception as e:
            print(f"Failed to parse time from: {subnav[i].strip()} | Error: {e}")
            timest[i] = datetime(1900, 1, 1, tzinfo=timezone.utc)  # fallback or flag
...
def _dgs_proc_Langseth(path):
    """Read DGS laptop data from Langseth, which has a prefix of dgsdata yyyy:ddd:hh:mm:ss.ssss and some random errors of misalign colums"""
    cleaned, skipped = [], 0

    with open(path, 'r', encoding='utf-8', errors='ignore') as f:
        for ln_no, raw in enumerate(f, 1):
            s = raw.lstrip()  # allow leading spaces before 'dgsdata'
            ...
            if s.startswith('dgsdata'):
                ...
                tail = s.rstrip('\n')
            # Skip lines that are not 26 fields and report where are them
            fields = tail.split(',')
            if len(fields) != 26:
...
def _dgs_raw_general(path):
    ...
    if conv_times:  # probably UTC stamps
        raw_ts = dat['timestamp'].astype(str)
    ...
    dat['date_time'] = new_dates[valid].values
...
def _dgs_raw_Sikuliaq(path):
    """Read raw gravity file for R/V Sikuliaq."""
    dat = pd.read_csv(path, comment='#', delimiter='\t', names=[
        'logname', 'timestamp', 'raw'
    ])
