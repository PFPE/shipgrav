...
class gravDataTestCase(unittest.TestCase):
    def setUp(self):  # actions to take before running each test: load in some test data
        ...
        data['tsec'] = [e.timestamp() for e in data['date_time']]  # get posix timestamps
        data['grav'] = data['rgrav'] + 969143
        self.data = data

    def test_longman(self):
        lt = sgg.longman_tide_pred(self.data['lon'],self.data['lat'],self.data['date_time'])
    ...
    def test_up_vecs(self):
        ...
        up_vecs = sgg.up_vecs(1, lat_corr, acrss, along, 0, 240, 0.7071, 240, 0.7071)
