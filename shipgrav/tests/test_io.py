...
class ioTestCase(unittest.TestCase):
    ...
    def test_read_nav(self):  # NOTE only tests one of the ship functions
        ...
        nav = sgi.read_nav('Thompson', f'{self.ex_files}'+os.sep+'TN400_nav.Raw')
        self.assertEqual(nav.iloc[0].time_sec, 1647129603)
        self.assertTrue(nav.iloc[0].lon + 118.6524 < 0.001)

    def test_read_bgm_rgs(self):
        bgm = sgi.read_bgm_rgs(f'{self.ex_files}'+os.sep+'AT05_01_bgm.RGS', 'Atlantis')
    ...
    def test_read_bgm_raw(self):
        bgm = sgi.read_bgm_raw(f'{self.ex_files}'+os.sep+'TN400_bgm.Raw', 'Thompson')
    ...
    def test_read_dgs_dat(self):
        dgs = sgi.read_dgs_laptop(f'{self.ex_files}'+os.sep+'DGStest_laptop.dat', 'DGStest')
    ...
    def test_read_dgs_raw(self):
        dgs = sgi.read_dgs_raw(f'{self.ex_files}'+os.sep+'SR2312_dgs_raw.txt', 'Ride')
        self.assertEqual(
