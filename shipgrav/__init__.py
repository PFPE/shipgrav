...
shipgrav is a Python package with utilities for reading and processing marine gravity data from UNOLS ships. At time of writing, the UNOLS fleet is transitioning away from BGM3 gravimeters to DGS AT1M meters managed by the Potential Field Pool Equipment (PFPE). shipgrav is able to read files from both types of meters, as well as navigation data and other vessel feeds.

DGS gravimeters output two types of files: serial, or 'raw' files; and 'laptop' files. Raw files are written from the serial port, and contain counts values that can be calbrated to retrieve the gravity signal. In this documentation we use 'serial' and 'raw' interchangeably.  What we refer to as laptop files are lightly processed onboard the meter and output with biased gravity values alongside other information.
...
You can organize your data however you like; shipgrav does not care as long as you tell it where to look. However, the example scripts are all set up using a particular organization system that you may like to emulate. You can see this structure in ``example-scripts/data``. 
...
The scripts in the ``example-scripts`` directory use sample files to run through some common workflows. The sample data files are not available on github because of file size limits but we're working on making them available for download elsewhere.
